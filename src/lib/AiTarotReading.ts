...
const groq = new Groq({
  apiKey: import.meta.env.VITE_DEEPSEEK_AI,
...
class AiTarotReadingService {
  ...
  constructor() {
    this.apiKey = import.meta.env.VITE_DEEPSEEK_AI;
    if (!this.apiKey) {
  ...
  async generateTarotReading(
    ...
    try {
      ...
      const cardTitles =
        request.cardTitles.length === 6
          ? request.cardTitles
          : this.DEFAULT_CARD_TITLES;
      ...
      for (let i = 0; i < 6; i++) {
        ...
        console.log(
          `🔮 Processing Card ${i + 1}: ${card.name} - "${question}"`,
        );
...
Write each interpretation as a flowing, personalized reading that directly answers the question asked.`;
  ...
  async getReadingSession(
    sessionId: string,
  ): Promise<TarotReadingSession | null> {
    try {
