...
if (!supabaseServiceRoleKey) {
  console.warn(
    "Missing VITE_SUPABASE_SERVICE_ROLE_KEY - Admin operations will not be available",
  );
}

// The project ref from the Supabase URL, e.g. "sb-ccufxormqzvytjhvpvzx".
// Each client must use its own auth storage key to avoid the "Multiple
// GoTrueClient instances" warning (both would otherwise share the default
// `sb-{ref}-auth-token` localStorage key).
const supabaseProjectRef =
  supabaseUrl.match(/https?:\/\/([^.]+)\.supabase\.co/)?.[1] ?? "app";
...
// It uses a dedicated storage key because it is a separate GoTrue instance.
export const supabaseAdmin = supabaseServiceRoleKey
  ? createClient(supabaseUrl, supabaseServiceRoleKey, {
      auth: {
        autoRefreshToken: false,
        persistSession: false,
        storageKey: `sb-${supabaseProjectRef}-admin-auth-token`,
      },
    })
  : null;
...
export const adminDb = supabaseAdmin
  ? {
      ...
      auth: {
        createUser: supabaseAdmin.auth.admin.createUser.bind(
          supabaseAdmin.auth.admin,
        ),
        deleteUser: supabaseAdmin.auth.admin.deleteUser.bind(
          supabaseAdmin.auth.admin,
        ),
        listUsers: supabaseAdmin.auth.admin.listUsers.bind(
          supabaseAdmin.auth.admin,
        ),
        updateUserById: supabaseAdmin.auth.admin.updateUserById.bind(
          supabaseAdmin.auth.admin,
        ),
        getUserById: supabaseAdmin.auth.admin.getUserById.bind(
          supabaseAdmin.auth.admin,
        ),
      },
    }
...
export const STORAGE_BASE_URL =
  "https://rhuxiyqliygmogxcwltv.supabase.co/storage/v1/object/public/";
