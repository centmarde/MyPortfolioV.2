...
export function ChromaticImage({
  ...
  useEffect(() => {
    ...
    const updatePointer = (event: PointerEvent) => {
      ...
      let active = false;
      if (hotspots) {
        // Activate only within a radius around each configured hotspot
        // (e.g. the CTA buttons). Everywhere else the effect stays off.
        const radius = hotspotRadius * hotspotRadius;
        const elements = document.querySelectorAll<HTMLElement>(hotspots);
  ...
  }, [backgroundColor, displacement, chromaticShift, src, tilt, zoom, scope, hotspots, hotspotRadius]);
