...
import { tarotCards } from "@/components/composables/tarotConstant";
...
export const TarotCardsResults: React.FC<TarotCardsResultsProps> = ({
  ...
  const { getMyDecks, deleteDeck, isLoading, error } =
    useTarotCardsDataStore();
  ...
  // State for the user's decks
  const [decks, setDecks] = React.useState<TarotCardsDeck[]>([]);
  const [loadingDecks, setLoadingDecks] = React.useState(true);
  const [hasUserDecksInDB, setHasUserDecksInDB] = React.useState(false);
  const [latestDeck, setLatestDeck] = React.useState<TarotCardsDeck | null>(
    null,
  );
  ...
  const handleCreateReading = async () => {
    ...
    try {
      ...
      // If the current reading slot is already in use, remove it before
      // creating a new one to avoid duplicates.
      if (shouldAllowNewReading(latestDeck) && latestDeck) {
        console.log(
          `🔮 Removing existing deck before creating a new reading: ${latestDeck.id}`,
        );
        const ok = await deleteDeck(latestDeck.id);
        if (!ok) {
          console.error(
  ...
  // Load the user's decks on mount
  useEffect(() => {
    let isMounted = true;
    ...
    const loadDecks = async () => {
      try {
        ...
        const userDecks = await getMyDecks();
        ...
        if (isMounted) {
          ...
          // Track latest deck (sorted desc by created_at)
          setLatestDeck(userDecks[0] ?? null);

          // Filter only complete readings
          setDecks(userDecks.filter(isCompleteReading));
        }
      } catch (error) {
        if (isMounted) {
  ...
  const renderDeckCard = (deck: TarotCardsDeck) => {
  ...
  const renderEmptySection = () => {
    const emptyMessage = hasUserDecksInDB
      ? `${bfName} has readings, but none are complete yet`
      : `No ${bfName}'s readings found`;
    ...
    return (
      <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
        ...
        {!hasUserDecksInDB && (
          ...
          >
            🔮 Loaded 0 user decks from cache
          </p>
        )}
        ...
        >
          ...
          >
            ...
            >
              ...
              {bfName}'s Readings ({decks.length})
            </h2>

            {shouldAllowNewReading(latestDeck) ? (
              <p
